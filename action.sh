...
ExampleConfigFile=$MODDIR/SystemlessDebloater.cfg
...
then
	...
	TmpConfigFile=$MODDIR/TmpSystemlessDebloater.cfg
...
if [ -n "$DebloatedList" ]
then
  su -lp 2000 -c "cmd notification post -S bigtext -t 'Systemless Debloater' 'Tag' 'Reboot to complete debloating'" 1>/dev/null
  echo "'Reboot to complete debloating"
fi
